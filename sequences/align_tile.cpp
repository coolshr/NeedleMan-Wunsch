...
#pragma omp parallel for simd
    ...
    double time = tock(&t);
    cout << time * 1000 << "\n";
    cout << (6*sizea*sizeb / time) * 1e-9 << "\n";
    cout << (14*sizea*sizeb/time) * 1e-9 << "\n";
    // cout << "DP MATRIX\n";
...
//         {
//             // cout << sum << " " << i << " " << j << "\n";
//             int tilesizeX = (tile_size <= (sizea + 1 - j)) * tile_size + (tile_size > (sizea + 1 - j)) * (sizea + 1 - j);
//             int tilesizeY = (tile_size <= (sizeb + 1 - i)) * tile_size + (tile_size > (sizeb + 1 - i)) * (sizeb + 1 - i);
